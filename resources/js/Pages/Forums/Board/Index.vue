...
<template>
  <AuthenticatedLayout>
    ...
    <Card class="w-full text-left flex items-center justify-between p-4">
      ...
      <Link :href="route('forums.board.thread.create', board)" class="p-3 rounded bg-neutral-600 border border-neutral-200/20 shadow-slim text-white font-medium text-md">
        Create Thread
      </Link>
    </Card>

    <div class="w-full grid gap-y-4 pt-4">
        <Card v-if="$page.props.board.threads.length > 0" class="bg-neutral-700" divide>
